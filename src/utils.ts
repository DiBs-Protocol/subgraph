...
export function getRoutes(
  token: Address,
  target: Address
): Array<Router__getAmountsOutInputRoutesStruct> {
  const pathToTarget = PathToTarget.load(token.toHex() + "-" + target.toHex());
  if (pathToTarget == null) {
    return [];
  } else {
    const routes: Array<Router__getAmountsOutInputRoutesStruct> = new Array();
    let inToken = token;
    for (let i = 0; i < pathToTarget.path.length; i++) {
      ...
      const route: Router__getAmountsOutInputRoutesStruct = new Router__getAmountsOutInputRoutesStruct();
